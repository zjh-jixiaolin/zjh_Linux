...
`chmod` 目录设置 **特殊权限**，只需在 `3` 个数字之前添加一个数字位，用来放置给文件或目录设定特殊权限。如`chmod 4755`表示 `-rwsr-xr-x` 。`SUID、SGID、SBIT`分别对应的数字如下：
...
除了赋予 `chmod` 命令 `4` 个数字设定 **特殊权限**。还可以使用 **字母**的形式，如使用 `u + s` 给文件赋予`SUID` 权限，通过 `g + s` 给文件或目录赋予`SGID` 权限，通过 ` o+t` 给目录赋予 `SBIT` 权限。

```bash
[root@localhost ~]# ll test
> -rwxr-xr-x. 1 root root 0 May 10 15:14 test
# 设置 特殊权限
[root@localhost ~]# chmod u+s,g+s,o+t test
[root@localhost ~]# ll test
> -rwsr-sr-t. 1 root root 0 May 10 15:14 test
...
>如上所示可知，使用 `chmod` 命令给文件或目录赋予特殊权限时，原文件或目录存在 `x`权限会被替换成 `s` 或 `t`，而当我们使用 `chmod` 命令**消除**文件或目录的特殊权限时，原本的`x` 权限又会显示出来。因为无论是 `SUID、SGID 或 SBIT` ，它们**只针对**具有 `x` 权限的文件或目录有效。
>
>没有 `x` 权限的文件或目录，即便赋予特殊权限，也无法发挥它们的功能，毫无意义，如下是我们给不具有 `x` 权限的文件或目录，即便赋予 **特殊权限**，也无法发挥功能，没有任何意义
>
>```bash
>[root@localhost ~]# ll test
>> -rw-rw-rw-. 1 root root 0 May 10 15:42 test
># 赋予特殊权限
>[root@localhost ~]# chmod 7666 test
>[root@localhost ~]# ll test
>> -rwSrwSrwT. 1 root root 0 May 10 15:42 test
>```
>
>如上可知结论，相应的权限位会被标记为 `S` 和 `T`，指的是设置的`SUID、SGID、SBIT` 无意义。
...
内容暂时用不上，待补充，关于[学习链接](http://c.biancheng.net/view/875.html)
